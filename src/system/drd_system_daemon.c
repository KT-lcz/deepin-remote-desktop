...
#define DRD_DISPLAY_MANAGER_OBJECT_PATH "/org/deepin/DisplayManager"
...
typedef struct _DrdRemoteClient
{
    ...
    gboolean assigned;
    gchar *user_name;
    guint32 reserved_uid;
    gboolean use_system_credentials;
    guint handover_count;
    gint64 last_activity_us; // TODO delete
...
typedef struct
{
    ...
    GDBusObjectManager *display_manager_object_manager;
...
static void drd_system_daemon_handle_display_manager_removed_object_path(DrdSystemDaemon *self,
                                                                         const gchar *removed_object_path);

static void drd_system_daemon_on_display_manager_object_removed(GDBusObjectManager *manager, GDBusObject *object,
                                                                gpointer user_data);
                                                           ...
                                                           gchar **routing_token_out)
{
    ...
    while (TRUE)
    {
        routing_token_value = g_random_int();
        ...
        handover_dbus_path = get_dbus_path_from_routing_token(routing_token_value);
        if (handover_dbus_path == NULL)
        {
            continue;
        }

        if (!g_hash_table_contains(self->remote_clients, handover_dbus_path))
        {
            break;
        }
    }

    routing_token = g_strdup_printf("%u", routing_token_value);
    if (routing_token == NULL)
    {
        return FALSE;
    }

    *remote_id_out = g_steal_pointer(&handover_dbus_path);
...
static gboolean drd_system_daemon_get_caller_identity(DrdSystemDaemon *self, GDBusMethodInvocation *invocation,
                                                      guint32 *uid_out, gchar **user_out)
{
    g_return_val_if_fail(DRD_IS_SYSTEM_DAEMON(self), FALSE);
    g_return_val_if_fail(invocation != NULL, FALSE);
    g_return_val_if_fail(uid_out != NULL, FALSE);
    g_return_val_if_fail(user_out != NULL, FALSE);

    const gchar *sender = g_dbus_method_invocation_get_sender(invocation);
    if (sender == NULL || *sender == '\0')
    {
        DRD_LOG_WARNING("Failed to resolve DBus sender for request");
        return FALSE;
    }
    ...
    if (connection == NULL)
    {
        DRD_LOG_WARNING("Failed to resolve DBus connection for %s", sender);
        return FALSE;
    }

    g_autoptr(GError) error = NULL;
    g_autoptr(GVariant) reply =
            g_dbus_connection_call_sync(connection, "org.freedesktop.DBus", "/org/freedesktop/DBus",
                                        "org.freedesktop.DBus", "GetConnectionUnixUser", g_variant_new("(s)", sender),
                                        G_VARIANT_TYPE("(u)"), G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
    if (reply == NULL)
    {
        DRD_LOG_WARNING("Failed to query Unix user for %s: %s", sender, error != NULL ? error->message : "unknown");
        return FALSE;
    }
    ...
    g_autofree gchar *buf = g_malloc0((gsize) buf_len);
    struct passwd pwd;
    struct passwd *pwd_result = NULL;
    int rc = getpwuid_r((uid_t) uid, &pwd, buf, (size_t) buf_len, &pwd_result);
    if (rc != 0 || pwd_result == NULL || pwd_result->pw_name == NULL)
    {
        DRD_LOG_WARNING("Failed to resolve username for uid %u", uid);
        return FALSE;
    }

    *uid_out = uid;
    *user_out = g_strdup(pwd_result->pw_name);
    return *user_out != NULL && **user_out != '\0';
}
...
static void drd_remote_client_free(DrdRemoteClient *client)
{
    ...
    g_clear_pointer(&client->user_name, g_free);
                                                                    ...
                                                                    gpointer user_data)
{
    ...
    if (g_variant_lookup(changed_properties, "user_name", "s", &user_name))
    {
        DRD_LOG_MESSAGE("changed user name is %s", user_name);
        watch_client->user_name = g_strdup(user_name);
        struct passwd *pwd = getpwnam(user_name);
        if (pwd != NULL)
            watch_client->reserved_uid = pwd->pw_uid;
    }
    ...
    {
        DRD_LOG_MESSAGE("watch client session changed");
    ...
    if (new_client && new_client->routing)
    {
        g_clear_pointer(&watch_client->greeter_client_id, g_free);
        watch_client->greeter_client_id = g_strdup(new_client->routing->routing_token);
    }
...
static gboolean drd_system_daemon_watch_display_session(DrdRemoteClient *client, const gchar *session_path)
{
    ...
    g_clear_pointer(&client->login1_session_id, g_free);
    // drd_dbus_lightdm_remote_display_factory_session_dup_session_id generated by org.deepin.DisplayManager.xml, don't
    // edit.
    client->login1_session_id =
            drd_dbus_lightdm_remote_display_factory_session_dup_session_id(client->lightdm_session_proxy);
 ...
 * 外部接口：GLib g_hash_table_lookup；日志 DRD_LOG_WARNING。
 */
static DrdRemoteClient *drd_system_daemon_find_client_by_token(DrdSystemDaemon *self, const gchar *routing_token)
{
 ...
 * 信息。 外部接口：GLib g_hash_table_contains/g_object_set_data；GDBus
                                                  ...
                                                  DrdRoutingTokenInfo *info)
{
    ...
    if (self->remote_clients != NULL && g_hash_table_size(self->remote_clients) >= DRD_SYSTEM_MAX_PENDING_CLIENTS)
    {
        DRD_LOG_WARNING("Handover table full (%u >= %u), rejecting new client", g_hash_table_size(self->remote_clients),
    ...
    {
        handover_dbus_path =
                g_strdup_printf("%s%s", DRD_REMOTE_DESKTOP_HANDOVER_SESSION_PATH_PREFIX, info->routing_token);
                                           ...
                                           GError **error)
{
    ...
    DRD_LOG_MESSAGE("Registered new handover client (total=%u)", drd_system_daemon_get_remote_client_count(self));
...
static gboolean drd_system_daemon_on_session_ready(DrdRdpListener *listener, DrdRdpSession *session, gpointer user_data)
{
    ...
    if (drd_rdp_listener_is_single_login(self->listener))
    {
        ...
        {
            client->user_name = g_strdup(auth_username);
            drd_system_daemon_touch_client(client);
        }
                                                          ...
                                                          GDBusMethodInvocation *invocation, gpointer user_data)
{
     ...
     */
    DrdSystemDaemon *self = DRD_SYSTEM_DAEMON(user_data);
    guint32 caller_uid = 0;
    g_autofree gchar *caller_name = NULL;
    if (!drd_system_daemon_get_caller_identity(self, invocation, &caller_uid, &caller_name))
    {
    ...
    DrdRemoteClient *selected = NULL;
    gint64 selected_activity = G_MAXINT64;
    gboolean is_lightdm = g_strcmp0(caller_name, "lightdm") == 0;
    ...
    while (g_hash_table_iter_next(&iter, NULL, &value))
    {
        ...
        if (is_lightdm)
        {
            if (candidate->handover_count == 0 && candidate->user_name == NULL && candidate->connection != NULL)
            {
                selected = candidate;
                break;
            }
        ...
        {
            if (candidate->user_name != NULL && g_strcmp0(candidate->user_name, caller_name) == 0)
            {
                selected = candidate;
                break;
            }
        ...
        //
        // gint64 activity = candidate->last_activity_us > 0 ? candidate->last_activity_us : 0;
        // if (selected == NULL || activity < selected_activity)
        // {
        //     selected = candidate;
    ...
    if (selected == NULL)
    {
        g_dbus_method_invocation_return_error(invocation, G_IO_ERROR, G_IO_ERROR_NOT_FOUND,
                                              "No matching RDP handover requests");
        return TRUE;
    }

    selected->assigned = TRUE;
    selected->reserved_uid = caller_uid;
    drd_system_daemon_touch_client(selected);
    drd_dbus_remote_desktop1_remote_desktop1_remote_login_complete_request_handover(interface, invocation,
                                                                                    selected->handover_dbus_path);
    DRD_LOG_MESSAGE("Dispatching handover client %s to %s", selected->handover_dbus_path, caller_name);
    return TRUE;
}
...
static void drd_system_daemon_reset_bus_context(DrdSystemDaemon *self)
{
     ...
     */
    g_clear_pointer(&self->bus.display_manager_object_manager, g_object_unref);
...
void drd_system_daemon_stop(DrdSystemDaemon *self)
{
     ...
     * 逻辑：重置 DBus 上下文、停止监听器、清空客户端哈希表，并请求主循环退出。
     * 参数：self system 守护实例。
     * 外部接口：内部
...
static void drd_system_daemon_dispose(GObject *object)
{
     ...
     * 逻辑：调用 stop 清理运行态，再释放 TLS/运行时/配置与主循环引用，销毁哈希表，最后交由父类 dispose。
     * 参数：object 基类指针，期望为 DrdSystemDaemon。
     * 外部接口：GLib g_clear_object/g_clear_pointer/g_hash_table_destroy；内部 drd_system_daemon_stop。
     */
...
static void drd_system_daemon_init(DrdSystemDaemon *self)
{
     ...
     * 逻辑：清空总线上下文、创建客户端哈希表，主循环置空。
     * 参数：self system 守护实例。
     * 外部接口：GLib g_hash_table_new_full。
     */
    ...
    self->bus.display_manager_object_manager = NULL;
    self->remote_clients =
...
static void drd_system_daemon_handle_display_manager_removed_object_path(DrdSystemDaemon *self,
                                                                         const gchar *removed_object_path)
{
    if (!DRD_IS_SYSTEM_DAEMON(self) || self->remote_clients == NULL)
    {
        return;
    }

    if (removed_object_path == NULL || *removed_object_path == '\0')
    {
        return;
    }

    DrdRemoteClient *matched = NULL;
    GHashTableIter iter;
    gpointer value = NULL;
    g_hash_table_iter_init(&iter, self->remote_clients);
    while (g_hash_table_iter_next(&iter, NULL, &value))
    {
        DrdRemoteClient *candidate = value;
        if (candidate != NULL && candidate->lightdm_session_path != NULL &&
            g_strcmp0(candidate->lightdm_session_path, removed_object_path) == 0)
        {
            matched = candidate;
            break;
        }
    ...
    if (matched != NULL)
    {
        DRD_LOG_MESSAGE("DisplayManager removed %s, removing client %s", removed_object_path,
                        matched->handover_dbus_path != NULL ? matched->handover_dbus_path : "unknown");
        drd_system_daemon_remove_client(self, matched);
    }
}
                                                                ...
                                                                gpointer user_data)
{
    (void) manager;
    DrdSystemDaemon *self = DRD_SYSTEM_DAEMON(user_data);
    if (!DRD_IS_SYSTEM_DAEMON(self) || object == NULL)
    {
        return;
    }

    const gchar *removed_object_path = g_dbus_object_get_object_path(object);
    DRD_LOG_MESSAGE("object removed:%s", removed_object_path);
    drd_system_daemon_handle_display_manager_removed_object_path(self, removed_object_path);
}
...
static gboolean drd_system_daemon_start_bus(DrdSystemDaemon *self, GError **error)
{
    ...
    g_autoptr(GError) local_error = NULL;
    self->bus.display_manager_object_manager = drd_dbus_lightdm_object_manager_client_new_sync(
            self->bus.connection, G_DBUS_OBJECT_MANAGER_CLIENT_FLAGS_DO_NOT_AUTO_START,
            DRD_LIGHTDM_REMOTE_FACTORY_BUS_NAME, DRD_DISPLAY_MANAGER_OBJECT_PATH, NULL, &local_error);
    if (self->bus.display_manager_object_manager == NULL)
    {
        DRD_LOG_WARNING("Failed to create DisplayManager object manager at %s: %s", DRD_DISPLAY_MANAGER_OBJECT_PATH,
                        local_error != NULL ? local_error->message : "unknown");
    }
    else
    {
        g_signal_connect_object(self->bus.display_manager_object_manager, "object-removed",
                                G_CALLBACK(drd_system_daemon_on_display_manager_object_removed), self, 0);
    }
                                                    ...
                                                    gpointer user_data)
{
    ...
    if (!drd_system_daemon_get_caller_identity(self, invocation, &caller_uid, &caller_name) ||
        client->reserved_uid != caller_uid)
    {
        DRD_LOG_MESSAGE("caller uid is %d", caller_uid);
        DRD_LOG_MESSAGE("reserved uid is %d", client->reserved_uid);
    ...
    {
        // session已经重定向到handover进程
        DRD_LOG_MESSAGE("client session is NULL");
        DrdRemoteClient *redirected_client = NULL;
        gboolean greeter_client_redirected = FALSE;
        if (client->greeter_client_id != NULL)
        {
            DRD_LOG_MESSAGE("greeter client id is %s", client->greeter_client_id);
            redirected_client = drd_system_daemon_find_client_by_token(client->daemon, client->greeter_client_id);
            greeter_client_redirected = TRUE;
            g_clear_pointer(&client->greeter_client_id, g_free);
        }
        else
        {
            redirected_client = client;
        }
        if (!redirected_client)
        {
            DRD_LOG_MESSAGE("redirected_client not found");
            g_dbus_method_invocation_return_error(invocation, G_IO_ERROR, G_IO_ERROR_FAILED,
                                                  "Failed to redirect client session: not found client need redirect");
            return TRUE;
        }


        const gboolean has_routing_token =
                redirected_client->routing != NULL && redirected_client->routing->routing_token != NULL;
        if (has_routing_token)
        {
            DRD_LOG_MESSAGE("routing token is %s", redirected_client->routing->routing_token);
            drd_dbus_remote_desktop1_remote_desktop1_handover_session_emit_redirect_client(
                    redirected_client->handover_iface, redirected_client->routing->routing_token, one_time_auth_token);
            if (greeter_client_redirected)
            {
                if (client->routing != NULL)
                {
                    g_clear_pointer(&client->routing->routing_token, g_free);
                    client->routing->routing_token = g_strdup(redirected_client->routing->routing_token);
                    client->routing->requested_rdstls = redirected_client->routing->requested_rdstls;
                }
                if (redirected_client->lightdm_session_proxy != NULL)
                    g_signal_handlers_disconnect_by_data(redirected_client->lightdm_session_proxy, redirected_client);
                drd_system_daemon_remove_client(redirected_client->daemon, redirected_client);
            }
        }
        else
        {
