...
typedef struct
{
    ...
    guint display_manager_interfaces_removed_sub_id;
...
static void drd_system_daemon_on_display_manager_interfaces_removed(GDBusConnection *connection,
...
static gboolean drd_system_daemon_watch_display_session(DrdRemoteClient *client, const gchar *session_path)
{
    ...
    g_clear_pointer(&client->login1_session_id, g_free);
    // drd_dbus_lightdm_remote_display_factory_session_dup_session_id generated by org.deepin.DisplayManager.xml, don't edit.
...
static void drd_system_daemon_reset_bus_context(DrdSystemDaemon *self)
{
     ...
     */
    if (self->bus.display_manager_interfaces_removed_sub_id != 0 && self->bus.connection != NULL)
    {
        g_dbus_connection_signal_unsubscribe(self->bus.connection, self->bus.display_manager_interfaces_removed_sub_id);
        self->bus.display_manager_interfaces_removed_sub_id = 0;
    }
                                                                    ...
                                                                    gpointer user_data)
{
    ...
    DrdSystemDaemon *self = DRD_SYSTEM_DAEMON(user_data);
    if (!DRD_IS_SYSTEM_DAEMON(self) || self->remote_clients == NULL || parameters == NULL)
    {
        return;
    }

    const gchar *removed_object_path = NULL;
    gchar **removed_interfaces = NULL;
...
static gboolean drd_system_daemon_start_bus(DrdSystemDaemon *self, GError **error)
{
    ...
    self->bus.display_manager_interfaces_removed_sub_id = g_dbus_connection_signal_subscribe(
            self->bus.connection, NULL, "org.freedesktop.DBus.ObjectManager", "InterfacesRemoved",
            DRD_DISPLAY_MANAGER_OBJECT_PATH, NULL, G_DBUS_SIGNAL_FLAGS_NONE,
            drd_system_daemon_on_display_manager_interfaces_removed, self, NULL);
    if (self->bus.display_manager_interfaces_removed_sub_id == 0)
    {
        DRD_LOG_WARNING("Failed to subscribe to DisplayManager InterfacesRemoved at %s",
                        DRD_DISPLAY_MANAGER_OBJECT_PATH);
    }
