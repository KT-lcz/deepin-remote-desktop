...
static gchar *get_dbus_path_from_routing_token(guint32 routing_token)
{
    ...
    return g_strdup_printf("%s/%u", DRD_REMOTE_DESKTOP_HANDOVERS_OBJECT_PATH,
                           routing_token);
}

static gboolean drd_system_daemon_is_graphical_session_type(const gchar *type)
{
    return g_strcmp0(type, "x11") == 0 || g_strcmp0(type, "wayland") == 0 ||
           g_strcmp0(type, "mir") == 0;
}

static gboolean drd_system_daemon_collect_local_graphical_sessions(
        DrdSystemDaemon *self, const gchar *username,
        ...
        GPtrArray **sessions_out, GError **error)
{
    ...
    g_clear_error(&local_error);
    if (!drd_dbus_logind_manager_call_list_sessions_sync(
            manager, &sessions, NULL, &local_error))
    {
        if (error != NULL)
        {
            g_propagate_error(error, g_steal_pointer(&local_error));
    ...
    while (g_variant_iter_loop(&iter, "(susso)", &session_id, &uid, &user,
                               &seat, &path))
    {
        (void) uid;
        (void) seat;
        ...
        g_autoptr(DrdDBusLogindSession) session_proxy =
                drd_dbus_logind_session_proxy_new_for_bus_sync(
                        G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_DO_NOT_AUTO_START,
                        DRD_LOGIND_BUS_NAME, path, NULL, &local_error);
        ...
        if (type == NULL)
        {
            if (error != NULL)
            {
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                            "Session %s type unavailable", session_id);
            }
            return FALSE;
        }
...
static gboolean drd_system_daemon_terminate_local_graphical_sessions(
        DrdDBusLogindManager *manager,
        ...
        GError **error)
{
    ...
    for (guint i = 0; i < session_ids->len; i++)
    {
        ...
        g_autoptr(GError) local_error = NULL;
        if (!drd_dbus_logind_manager_call_terminate_session_sync(
                manager, session_id, NULL, &local_error))
        {
            if (error != NULL)
            {
                g_propagate_error(error, g_steal_pointer(&local_error));
        ...
        DRD_LOG_MESSAGE("terminated local session %s for user %s", session_id,
                        username);
    }
...
static gchar *get_routing_token_from_dbus_path(const gchar *id)
{
    ...
    {
        DRD_LOG_WARNING(
                "remote handover_dbus_path %s missing handover prefix %s", id,
                prefix);
        return NULL;
    }
    ...
    {
        DRD_LOG_WARNING(
                "remote handover_dbus_path %s missing routing token segment",
                id);
        return NULL;
    }
 ...
 */
static gboolean drd_system_daemon_generate_remote_identity(
        DrdSystemDaemon *self, gchar **remote_id_out,
...
static void drd_system_daemon_remove_client(DrdSystemDaemon *self,
                                            DrdRemoteClient *client);

static gboolean drd_system_daemon_queue_client(DrdSystemDaemon *self,
                                               DrdRemoteClient *client);

static void drd_system_daemon_unqueue_client(DrdSystemDaemon *self,
                                             DrdRemoteClient *client);

static gboolean drd_system_daemon_register_client(
        DrdSystemDaemon *self, GSocketConnection *connection,
...
drd_system_daemon_on_session_ready(DrdRdpListener *listener,
                                   DrdRdpSession *session, gpointer user_data);

static DrdRemoteClient *drd_system_daemon_find_client_by_token(
        DrdSystemDaemon *self, const gchar *routing_token);

static gboolean drd_system_daemon_on_start_handover(
        DrdDBusRemoteDesktopRdpHandover *interface,
        GDBusMethodInvocation *invocation, const gchar *username,
        const gchar *password, gpointer user_data);

static gboolean drd_system_daemon_on_take_client(
        DrdDBusRemoteDesktopRdpHandover *interface,
        GDBusMethodInvocation *invocation, GUnixFDList *fd_list,
        gpointer user_data);

static gboolean drd_system_daemon_on_get_system_credentials(
        DrdDBusRemoteDesktopRdpHandover *interface,
        GDBusMethodInvocation *invocation, gpointer user_data);

static void drd_system_daemon_on_bus_name_acquired(
 ...
 */
static void drd_system_daemon_prune_stale_pending_clients(
        DrdSystemDaemon *self, gint64 now_us)
{
    ...
    //                             candidate->handover_dbus_path != NULL ? candidate->handover_dbus_path : "unknown", idle_us / (gdouble) G_USEC_PER_SEC);
...
static void drd_remote_client_free(DrdRemoteClient *client)
{
    ...
    {
        g_signal_handlers_disconnect_by_data(client->lightdm_session_proxy,
                                             client);
    ...
    {
        g_object_set_data(
                G_OBJECT(client->connection), "drd-system-client", NULL);
        g_object_set_data(
                G_OBJECT(client->connection), "drd-system-keep-open", NULL);
...
static void drd_system_daemon_on_lightdm_session_properties_changed(
        ...
        gpointer user_data)
{
    ...
    DRD_LOG_MESSAGE("LightDM session %s client_id changed %s -> %s",
                    watch_client->lightdm_session_path != NULL ? watch_client->
                    lightdm_session_path : "unknown",
    ...
    if (new_client_id == NULL || watch_client->routing == NULL || watch_client->
        routing->routing_token == NULL)
    {
        return;
    }
    ...
    DrdRemoteClient *new_client = drd_system_daemon_find_client_by_token(
            watch_client->daemon, new_client_id);
    if (!new_client)
    {
    ...
    if (watch_client->routing != NULL)
    {
        ...
        watch_client->routing->routing_token = g_strdup(
                new_client->routing->routing_token);
        watch_client->routing->requested_rdstls = new_client->routing->
                requested_rdstls;
    }
    ...
    {
        g_object_set_data(
                G_OBJECT(watch_client->connection), "drd-system-client", NULL);
        g_object_set_data(
                G_OBJECT(watch_client->connection), "drd-system-keep-open",
                NULL);
    }
    ...
    {
        g_object_set_data(
                G_OBJECT(watch_client->connection), "drd-system-client",
                watch_client);
        g_object_set_data(
                G_OBJECT(watch_client->connection), "drd-system-keep-open",
                GINT_TO_POINTER(1));
    ...
    drd_dbus_remote_desktop_rdp_handover_emit_restart_handover(
            watch_client->handover_iface);
    // free new client

    if (new_client->lightdm_session_proxy != NULL)
    {
        g_signal_handlers_disconnect_by_data(new_client->lightdm_session_proxy,
                                             new_client);
    }
    drd_system_daemon_remove_client(watch_client->daemon, new_client);
}

static gboolean drd_system_daemon_watch_display_session(
        DrdRemoteClient *client, const gchar *session_path)
{
    ...
    while (g_hash_table_iter_next(&iter, NULL, &value))
    {
        ...
        DRD_LOG_MESSAGE("candidate session path is %s",
                        candidate->lightdm_session_path);
        if (candidate != NULL && g_strcmp0(session_path,
                                           candidate->lightdm_session_path) ==
            0)
        {
    ...
    client->lightdm_session_proxy =
            drd_dbus_lightdm_remote_display_factory_session_proxy_new_for_bus_sync(
                    G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_DO_NOT_AUTO_START,
                    DRD_LIGHTDM_REMOTE_FACTORY_BUS_NAME, session_path,
                    NULL, &error);
    if (client->lightdm_session_proxy == NULL)
    {
        DRD_LOG_WARNING("Failed to create LightDM session proxy for %s: %s",
                        session_path,
    ...
    g_signal_connect(client->lightdm_session_proxy, "g-properties-changed",
                     G_CALLBACK(
                         drd_system_daemon_on_lightdm_session_properties_changed
                     ), client);
    return TRUE;
}
        ...
        DrdSystemDaemon *self, const gchar *routing_token)
{
    ...
    success = g_ascii_string_to_unsigned(routing_token, 10, 1, G_MAXUINT32,
                                         &parsed_token, NULL);
    if (!success)
    {
    ...
    {
        DrdRemoteClient *candidate = value;
        if (candidate != NULL && candidate->routing != NULL && candidate->
            routing->routing_token != NULL && g_strcmp0(
                    candidate->routing->routing_token, routing_token) == 0)
        {
            return candidate;
        }
    }
                                               ...
                                               DrdRemoteClient *client)
{
    ...
    {
        DRD_LOG_WARNING(
                "Pending handover queue full (%u >= %u), cannot enqueue %s",
                pending,
                DRD_SYSTEM_MAX_PENDING_CLIENTS,
                client->routing->routing_token != NULL ? client->routing->
                routing_token : "unknown");
        return FALSE;
    }
                                            ...
                                            DrdRemoteClient *client)
{
    ...
    if (self->bus.handover_manager != NULL && client->handover_dbus_path !=
        NULL)
    {
        g_dbus_object_manager_server_unexport(self->bus.handover_manager,
        ...
        DrdRoutingTokenInfo *info)
{
    ...
    {
        if (g_ascii_string_to_unsigned(info->routing_token, 10, 1, G_MAXUINT32,
                                       &parsed_token_value, NULL))
        {
            handover_dbus_path = get_dbus_path_from_routing_token(
                    (guint32) parsed_token_value);
            routing_token = g_strdup(info->routing_token);
            if (handover_dbus_path != NULL && g_hash_table_contains(
                        self->remote_clients, handover_dbus_path))
            {
                DRD_LOG_WARNING(
                        "Routing token %s already tracked, generating a new one",
                        info->routing_token);
        ...
        {
            DRD_LOG_WARNING("Ignoring invalid routing token %s from peek",
                            info->routing_token);
        }
    }
    ...
    {
        if (!drd_system_daemon_generate_remote_identity(
                self, &handover_dbus_path, &routing_token))
        {
            DRD_LOG_WARNING(
                    "Unable to allocate remote identity for new handover client");
            drd_routing_token_info_free(client->routing);
            g_clear_object(&client->connection);
            g_free(client);
            return FALSE;
    ...
    client->handover_iface =
            drd_dbus_remote_desktop_rdp_handover_skeleton_new();
    g_signal_connect(client->handover_iface, "handle-start-handover",
                     G_CALLBACK(drd_system_daemon_on_start_handover),
                     client);
    g_signal_connect(client->handover_iface, "handle-take-client",
    ...
    client->object_skeleton = g_dbus_object_skeleton_new(
            client->handover_dbus_path);
    ...
    g_object_set_data(G_OBJECT(connection), "drd-system-keep-open",
                      GINT_TO_POINTER(1));

    g_hash_table_replace(self->remote_clients,
                         g_strdup(client->handover_dbus_path), client);
    if (!drd_system_daemon_queue_client(self, client))
    {
        DRD_LOG_WARNING(
                "Rejecting handover client %s because pending queue is full",
                client->handover_dbus_path);
        drd_system_daemon_remove_client(self, client);
        return FALSE;
    }

    const gchar *token_preview = client->routing != NULL && client->routing->
                                 routing_token != NULL
                                     ? client->routing->routing_token
                                     : "unknown";
    DRD_LOG_MESSAGE("Registered handover client %s (token=%s)",
                    client->handover_dbus_path, token_preview);
                                           ...
                                           GError **error)
{
    ...
    {
        // 发redirect PDU后重连
        DrdRemoteClient *existing = drd_system_daemon_find_client_by_token(
                self, info->routing_token);
        if (existing != NULL && existing->session == NULL)
        {
            g_clear_object(&existing->connection);
            existing->connection = g_object_ref(connection);
            g_object_set_data(
                    G_OBJECT(connection), "drd-system-client", existing);
            g_object_set_data(
                    G_OBJECT(connection), "drd-system-keep-open",
                    GINT_TO_POINTER(1));
 ...
 */
static gboolean drd_system_daemon_on_session_ready(
        DrdRdpListener *listener, DrdRdpSession *session, gpointer user_data)
{
    ...
    if (client->routing != NULL)
    {
        client->use_system_credentials =
                drd_rdp_session_client_is_mstsc(session) && !client->routing->
                requested_rdstls;
    }
    ...
    const DrdEncodingOptions *encoding_opts = drd_config_get_encoding_options(
            self->config);
    if (encoding_opts == NULL)
    {
        DRD_LOG_WARNING(
                "Encoding options unavailable for system session resolution");
        return FALSE;
    }

    guint32 target_width = encoding_opts->width;
    guint32 target_height = encoding_opts->height;
    if (drd_rdp_session_get_peer_resolution(session, &client_width,
                                            &client_height) && client_width > 0
        && client_height
        > 0)
    {
    ...
    if (drd_rdp_listener_is_single_login(self->listener))
    {
        ...
        gsize payload_len = 0;
        g_autofree gchar *shm_name = g_strdup_printf(
                "/drd-auth-%d-%u", (int) getpid(), g_random_int());
        int auth_fd = -1;
        ...
        if (auth_username == NULL || *auth_username == '\0' || auth_password ==
            NULL || *auth_password == '\0')
        {
            DRD_LOG_WARNING(
                    "single logon auth payload missing username/password");
            drd_system_daemon_touch_client(client);
            return FALSE;
        }
        ...
        if (!drd_system_daemon_collect_local_graphical_sessions(
                self, auth_username, &logind_manager, &local_sessions,
                &local_session_error))
        {
            DRD_LOG_WARNING("collect local sessions failed for user %s: %s",
                            auth_username,
                            local_session_error != NULL ? local_session_error->
                            message : "unknown");
            drd_system_daemon_touch_client(client);
            return FALSE;
        }
        ...
        if (ftruncate(auth_fd, (off_t) payload_len) != 0 || write(
                    auth_fd, auth_payload, payload_len) != (ssize_t)
            payload_len || lseek(auth_fd, 0, SEEK_SET) < 0)
        {
        ...
        if (!
            drd_dbus_lightdm_remote_display_factory_call_create_single_logon_session_sync(
                    self->remote_display_factory,
                    client->routing->routing_token, target_width, target_height,
                    fd_variant,
                    peer_address, fd_list, &session_path, &out_fd_list,
                    NULL, &error))
        {
    ...
    else
    {
        if (!
            drd_dbus_lightdm_remote_display_factory_call_create_remote_greeter_display_sync(
                    ...
                    client->routing->routing_token, target_width, target_height,
                    peer_address,
                    &session_path, NULL, &error))
        {
            DRD_LOG_WARNING("create remote display failed %s",
                            error != NULL ? error->message : "unknown");
            drd_system_daemon_touch_client(client);
            return FALSE;
        }
...
static gboolean drd_system_daemon_load_tls_material(
        DrdSystemDaemon *self, gchar **certificate, gchar **key,
        GError **error)
{
    /*
    ...
    if (self->tls_credentials == NULL)
    {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                            "TLS credentials unavailable");
        return FALSE;
    }

    return drd_tls_credentials_read_material(self->tls_credentials, certificate,
                                             key, error);
}

static gboolean drd_system_daemon_handle_request_handover(
        DrdDBusRemoteDesktopRdpDispatcher *interface,
        GDBusMethodInvocation *invocation, gpointer user_data)
{
    /*
     ...
     */
    DrdSystemDaemon *self = DRD_SYSTEM_DAEMON(user_data);
    drd_system_daemon_prune_stale_pending_clients(
            self, drd_system_daemon_now_us());
    ...
    drd_system_daemon_touch_client(client);
    drd_dbus_remote_desktop_rdp_dispatcher_complete_request_handover(
            interface, invocation, client->handover_dbus_path);
    DRD_LOG_MESSAGE("Dispatching handover client %s",
                    client->handover_dbus_path);
    return TRUE;
}
...
static void drd_system_daemon_reset_bus_context(DrdSystemDaemon *self)
{
    ...
    if (self->bus.dispatcher != NULL)
    {
        g_dbus_interface_skeleton_unexport(
                G_DBUS_INTERFACE_SKELETON(self->bus.dispatcher));
                                               ...
                                               gpointer user_data)
{
    ...
    DRD_LOG_WARNING("System daemon lost bus name %s, requesting shutdown",
                    name);
    /*
...
static void drd_system_daemon_init(DrdSystemDaemon *self)
{
    ...
    self->remote_clients = g_hash_table_new_full(
...
guint drd_system_daemon_get_remote_client_count(DrdSystemDaemon *self)
{
     ...
     */
    g_return_val_if_fail(DRD_IS_SYSTEM_DAEMON(self), 0);
    return self->remote_clients != NULL
               ? g_hash_table_size(self->remote_clients)
               : 0;
}

static gboolean drd_system_daemon_start_listener(
        DrdSystemDaemon *self, GError **error)
{
    /*
    ...
    drd_rdp_listener_set_delegate(self->listener, drd_system_daemon_delegate,
                                  self);
...
static gboolean drd_system_daemon_start_bus(DrdSystemDaemon *self,
                                            GError **error)
{
    /*
     * 功能：启动 DBus 端点并占用服务名。
    ...
    self->bus.bus_name_owner_id = g_bus_own_name_on_connection(
            self->bus.connection, DRD_REMOTE_DESKTOP_BUS_NAME,
            G_BUS_NAME_OWNER_FLAGS_REPLACE,
            drd_system_daemon_on_bus_name_acquired,
            drd_system_daemon_on_bus_name_lost, self,
            g_object_unref);
    ...
    self->bus.dispatcher =
            drd_dbus_remote_desktop_rdp_dispatcher_skeleton_new();
    g_signal_connect(self->bus.dispatcher, "handle-request-handover",
    ...
    if (!g_dbus_interface_skeleton_export(
            G_DBUS_INTERFACE_SKELETON(self->bus.dispatcher),
            self->bus.connection,
            DRD_REMOTE_DESKTOP_DISPATCHER_OBJECT_PATH, error))
    {
        return FALSE;
    }

    self->bus.handover_manager = g_dbus_object_manager_server_new(
        ...
        const gchar *password, gpointer user_data)
{
    ...
    if (!drd_system_daemon_load_tls_material(self, &certificate, &key,
                                             &io_error))
    {
    ...
    if (client->session != NULL)
    {
        ...
        DRD_LOG_MESSAGE("client session not NULL,routing token is %s",
                        client->routing->routing_token);
        if (!drd_rdp_session_send_server_redirection(
                client->session, client->routing->routing_token, username,
        ...
        drd_rdp_session_notify_error(client->session,
    ...
    else
    {
        ...
        {
            drd_dbus_remote_desktop_rdp_handover_emit_redirect_client(
                    interface, client->routing->routing_token,
                    username, password);
        }
        else
        {
            DRD_LOG_WARNING(
                    "StartHandover for %s missing routing token; skipping RedirectClient signal",
                    client->handover_dbus_path);
        }
    }

    drd_dbus_remote_desktop_rdp_handover_complete_start_handover(
            interface, invocation, certificate, key);
    ...
    DRD_LOG_MESSAGE("StartHandover acknowledged for %s",
                    client->handover_dbus_path);
    return TRUE;
}
        ...
        gpointer user_data)
{
    ...
    g_autoptr(GError) local_error = NULL;
    gint idx = g_unix_fd_list_append(out_list, g_socket_get_fd(socket),
                                     &local_error);
    if (idx == -1)
    {
    ...
    drd_dbus_remote_desktop_rdp_handover_complete_take_client(
            interface, invocation, out_list, handle);
    ...
    if (!drd_system_daemon_queue_client(self, client))
    {
        DRD_LOG_WARNING("Failed to requeue handover client %s, removing entry",
                        client->handover_dbus_path);
        drd_system_daemon_remove_client(self, client);
    }
    else
    {
        DRD_LOG_MESSAGE("Client %s ready for next handover stage",
                        client->handover_dbus_path);
    }
    // }
