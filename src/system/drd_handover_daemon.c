 ...
 */
static void drd_handover_daemon_dispose(GObject *object)
{
 ...
 * 外部接口：GLib 类型系统。
 */
static void drd_handover_daemon_class_init(DrdHandoverDaemonClass *klass)
{
 ...
 * 外部接口：无额外外部库。
 */
static void drd_handover_daemon_init(DrdHandoverDaemon *self)
{
 ...
 */
static gchar *drd_handover_daemon_generate_token(const gchar *prefix, gsize random_bytes)
{
    ...
    g_autoptr(GString) token = g_string_sized_new((prefix != NULL ? strlen(prefix) : 0) + random_bytes * 2);
    if (token == NULL)
    {
        return NULL;
    }
...
static gboolean drd_handover_daemon_refresh_nla_credentials(DrdHandoverDaemon *self, GError **error)
{
    ...
    {
        drd_handover_daemon_clear_nla_credentials(self);
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, "Failed to generate NLA credentials for handover");
        return FALSE;
    }
...
static gboolean drd_handover_daemon_start_session(DrdHandoverDaemon *self, GError **error)
{
    ...
    if (self->tls_credentials == NULL)
    {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, "TLS credentials unavailable for handover listener");
        return FALSE;
    }
    if (certificate == NULL || key == NULL)
    {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, "Dispatcher did not provide TLS material");
        return FALSE;
    }
    ...
    DRD_LOG_MESSAGE("StartHandover negotiated TLS (%zu bytes cert)", certificate != NULL ? strlen(certificate) : 0);
    return TRUE;
}
...
static gboolean drd_handover_daemon_take_client(DrdHandoverDaemon *self, GError **error)
{
    ...
    if (!drd_rdp_listener_adopt_connection(self->listener, g_steal_pointer(&connection), error))
    {
        return FALSE;
    }
                                                           ...
                                                           const gchar *username, const gchar *password)
{
    ...
    if (!drd_rdp_session_send_server_redirection(self->active_session, routing_token, username, password, certificate))
    {
...
gboolean drd_handover_daemon_start(DrdHandoverDaemon *self, GError **error)
{
    ...
    if (self->listener == NULL)
    {
        ...
        drd_rdp_listener_set_session_callback(self->listener, drd_handover_daemon_on_session_ready, self);
    }
